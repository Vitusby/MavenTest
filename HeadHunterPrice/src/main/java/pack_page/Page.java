...
public abstract class Page
{
	...
	{ 
		...
		this.driver.manage().timeouts().implicitlyWait(nTimeWait, TimeUnit.SECONDS);
	}
	...
	protected void CheckElementPresent (final int nKey, final String sLocator) throws ExceptFailTest
	{
	...
	protected void CheckTextForElement(WebElement wElement, String sText) throws ExceptFailTest
	{
		try
		{
		...
		{
			throw new ExceptFailTest("Значение текста в элементе не совпало с ожидаемым");	
		}	
	}

	// Проверка css элемента
	protected void CheckCssElement(WebElement wElement, String sPropertyName, String sExcpextedCssValue) throws ExceptFailTest
	{
		try
		{
			Assert.assertTrue(wElement.getCssValue(sPropertyName).equals(sExcpextedCssValue));
		}
		catch(AssertionError exc)
		{
			throw new ExceptFailTest("Значение параметра " + sPropertyName + " в элементе не совпало с ожидаемым");	
		}	
	}
	
	// Проверка атрибута элемента с ожиданнием
	protected void ChecksAtributeElement(final WebElement wElement, final String sAtributeName, final String sExcpextedAtributeValue) throws ExceptFailTest
	{
		boolean bFlag = false;
		WebDriverWait wWaitDriver = new WebDriverWait(driver, nTimeWait);
		try
		...
		catch(TimeoutException exc){System.out.println("Элемент c атрибутом "+sExcpextedAtributeValue+" не найден");}
	
		if(!bFlag)
		{
			throw new ExceptFailTest("Значение параметра " + sAtributeName + " в элементе не совпало с ожидаемым");	
		}	
	}
	
	// Проверка доступности/недоступности элемента 
	protected void CheckEnableElement(WebElement wElement, int nOperation) throws ExceptFailTest 
	{
		switch(nOperation)
		{
			case 1:
				if(!wElement.isEnabled())
				{
					...
					throw new ExceptFailTest("Элемент \"" + wElement.getText() + "\" не доступен");	
				}
				break;
			case 2:
				if(wElement.isEnabled())
				{
					...
					throw new ExceptFailTest("Элемент \"" + wElement.getText() + "\" доступен");	
				}
				break;
			default:
	...
	protected void CheckNotVisibleElement(String sLocator, String sName) throws ExceptFailTest 
	{
	...
	protected WebElement[] GetAllWebElements(final String sLocator) throws ExceptFailTest
	{
		...
		try
		{
			list = wWaitDriver.until(new ExpectedCondition<ArrayList<WebElement>>()
					{
				public ArrayList<WebElement> apply(WebDriver wd)
				{
					return  (ArrayList<WebElement>) wd.findElements(By.xpath(sLocator));
				}
					}								
										);
		}
		...
		{
			System.out.println("Не один элемент с локатором " + sLocator + " не найден");
			throw new ExceptFailTest("Не один элемент с локатором " + sLocator + " не найден");
		}
	...
	protected void CheckIsSelectedElement(final WebElement wElement, final Boolean bResult, String sName) throws ExceptFailTest
	{
		...
		try
		{
			wait.until(ExpectedConditions.elementSelectionStateToBe(wElement, bResult));
		}
		catch(TimeoutException exc)
		{
	...
	protected int GetIntFromTextInWebElement(WebElement wElement) throws ExceptFailTest
	{
		...
		{
			throw new ExceptFailTest("Не удалось перевести цену предложения в число");
		}
