...
public class ListenerThatHiglilightsElements extends AbstractWebDriverEventListener {
    ...
    {
    	//System.out.println("¬ыполн€етс€ подсветка"+ driver.findElement(by).getText());
    	//flash2(by, driver);
    }
	...
	public void onException(Throwable throwable, WebDriver driver)
	{
		if (throwable.getClass().equals(NoSuchElementException.class))
		{
			...
			if(lastFindBy.toString().equals("By.xpath: //textarea[contains(@class, 'x-form-focus')]"))
				return;
			if(lastFindBy.toString().equals("By.xpath: //div[@class='x-combo-list-inner']/div[contains(text(),'Ѕийск')]"))
				return;
			System.out.println("Ёлемент "+lNameWebElement.GetNameWebElement(lastFindBy.toString())+" не найден");
			try
			{
				wLog.WriteString(2, "Ёлемент "+lNameWebElement.GetNameWebElement(lastFindBy.toString())+" не найден. ");
			}
